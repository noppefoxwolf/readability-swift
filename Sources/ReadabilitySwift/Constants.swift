...
enum Constants {
    ...
    static var unlikelyCandidates: Regex<Substring> {
    ...
    static var byline: Regex<Substring> {
        /(?i)byline|author|dateline|writtenby|p-author/
    }

    static var videos: Regex<Substring> {
        #/(?i)(?:dailymotion|youtube|youtube-nocookie|player\.vimeo|v\.qq|bilibili|live\.bilibili)\.com/#
    }

    static var adWords: Regex<Substring> {
        #/(?i)^(?:ad(?:vertising|vertisement)?|pub(?:licité)?|werbung|广告|реклама|anuncio)$/#
    }

    static var loadingWords: Regex<Substring> {
        #/(?i)^(?:(?:loading|正在加载|загрузка|chargement|cargando)(?:…|\.\.\.)?)$/#
    }

    static var jsonLDArticleTypes: Regex<Substring> {
        #/(?i)^(?:Article|AdvertiserContentArticle|NewsArticle|AnalysisNewsArticle|OpinionNewsArticle|ReportageNewsArticle|ReviewNewsArticle|ScholarlyArticle|SocialMediaPosting|BlogPosting|LiveBlogPosting|DiscussionForumPosting|TechArticle|APIReference)$/#
    }
}
